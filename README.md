...
O caminho de IA usa a interface `AgentRuntime`, um adapter Mock determinístico e um adapter DeepSeek opcional. O Mock tem policy, budget, approval, provenance, quarentena de prompt injection e replay. O adapter DeepSeek funciona como uma ponte CVG `/v1` com health/manifest/tool-set estritos; não é alegado como protocolo nativo nem conectado a um processo externo neste workspace.
