...
| Contratos | `packages/contracts/src/index.ts`, 939 linhas, Zod e tipos compartilhados | Há envelope v1, IDs opacos, classes D0–D5, inputs e entidades; falta catálogo versionado completo, compatibilidade explícita, upcasters e validação uniforme de todas as respostas. |
...
| Deploy | `docker-compose.yml` contém somente PostgreSQL | Não há topologia completa web/API/worker/harness/proxy/telemetria, imagens endurecidas, secret mounts, limites de recursos, SBOM ou pipeline de promoção. |
| CI/CD | Não há `.github/workflows` no artifact | Lint, testes matriciais, segurança, migrations, container, SBOM e artefatos ainda não são gates remotos. |
