...
> Arquivado em `docs` a pedido do usuário após a auditoria; links das evidências ajustados. O texto abaixo registra o estado e a autorização da rodada de auditoria, não a gravação posterior deste arquivo. Original preservado em `artifacts/audit-2026-09-13/relatorio.md`.
...
- Corpus inventariado: **151 arquivos em `docs/`**, incluindo PRD, arquitetura, contratos, segurança, operação, 30 ADRs, 16 runbooks, provas históricas e plano de 54 tarefas/16 áreas. Lista, categorias, linhas e hashes em [inventario-docs.json](../artifacts/audit-2026-09-13/inventario-docs.json).
...
Isso comprova falha de feedback/revogação offline, **não bypass da autenticação do servidor**. Correção/aceite: distinguir saída local de revogação confirmada, tratar pendência/erro/retry e testar recarga. [Evidência](../artifacts/audit-2026-09-13/evidencias-visuais/behavior.json), [imagem](../artifacts/audit-2026-09-13/evidencias-visuais/offline-logout-reload.png).
...
É uma falha de continuidade do login, não uma prova de bypass MFA. Aceite: challenge, código, expiração, retry e retorno à sessão funcionando com contrato real. [Estados](../artifacts/audit-2026-09-13/evidencias-visuais/states.json), [imagem](../artifacts/audit-2026-09-13/evidencias-visuais/mfa-response-fixture.png).
...
Não foi demonstrada corrupção do ledger nem executado pagamento real. Aceite: PAID não aumenta saldo; parcial usa saldo remanescente; estornos seguem regra explícita. O ADR030 prepara o contrato e admite a migração futura — a deficiência está na adoção e na jornada. [Imagem](../artifacts/audit-2026-09-13/evidencias-visuais/finance-paid-fixture.png).
...
Aceite: aviso legível sem reduzir área útil, com foco/anúncio adequados; testar após clique, não apenas default. [Imagem](../artifacts/audit-2026-09-13/evidencias-visuais/mobile-toast.png), [medição](../artifacts/audit-2026-09-13/evidencias-visuais/states.json).
...
**Alta para operação prolongada; reproduzida localmente pelo revisor.** `packages/ops/src/index.ts:72,98,109–112,159–161` retém toda latência e log; maxSpans limita somente spans. 10000 chamadas com maxSpans=2 deixaram10000 latências e10000 logs; o Lead repetiu a reprodução na cópia: [medição](../artifacts/audit-2026-09-13/telemetry-probe.json). Cada coleta copia e ordena todo o histórico.
...
- **M01 — Contexto/data incorretos na agenda:** Unidade Sul selecionada apresenta “08 SET · UNIDADE CENTRO / Terça-feira” em13/09. `features/agenda/Agenda.tsx:36–37`. Dados vazios corretamente filtrados não corrigem o cabeçalho. [Imagem](../artifacts/audit-2026-09-13/evidencias-visuais/agenda-south.png).
...
| Estados adicionais de browser | **Defeitos reproduzidos** | Logout offline real; PAID/MFA com respostas interceptadas; toast e contexto reais. [comportamento](../artifacts/audit-2026-09-13/evidencias-visuais/behavior.json), [estados](../artifacts/audit-2026-09-13/evidencias-visuais/states.json) |
| Sentinel do repositório | **MATCH**,sem alteração observada | Hash de fontes desde início e sentinel completo incluindo ignorados/estado antes do crítico final. [resultado](../artifacts/audit-2026-09-13/sentinel-result.json) |
