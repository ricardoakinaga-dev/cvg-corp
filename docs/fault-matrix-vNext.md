...
| crash after dispatch | marcador + `OUTCOME_UNKNOWN` | PASS sintético | consultar receipt/reconciliar |
...
| worker lease loss | fencing/lease tests | PASS sintético | somente o fence vigente conclui |
...
Os testes existentes em `tests/integration/persistence.test.ts`, `tests/unit/integrations.test.ts` e `tests/unit/vnext.test.ts` são a evidência executável local. A ausência de serviço, provider, workload ou stores externos permanece explícita.
