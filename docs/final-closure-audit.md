...
**Revisão de referência do CVG:** `bf0cc506ff2fbbb99c77d334cfa60ba5921ffb22` (`feat: normalize operational read repositories`; diagnostics, hospitalization, medication, stock, finance, communication, knowledge, queue e AI sessions normalizados após `6b3df2f`/`135ae56` e o ciclo durável de break-glass, publicado em `main`)
...
**Checkpoint local final — 2026-09-10:** a rodada revalidou `126` testes (`125 pass`, `1 skip`), `test:database` `20/20`, lint, typecheck, build, `verify:pdp` (`68` operações, `70` regras, `6` policies canônicas, `12` domínios), migrations 029–030, usage/provenance, exportação governada, repositories normalizados de auditoria/encounters/clinical/diagnostics/care/medication/stock/finance/communication/knowledge/queue/AI, ciclo break-glass durável, provider HTTP loopback, Compose estrutural e os gates fail-closed. A crítica independente fresca anterior concluiu `FAIL_WITH_LIMITATIONS`; as tentativas fresh subsequentes sem parecer não são aprovação AAA.
...
| 8 Repositories normalizados | **PARTIAL** | leituras tipadas e transacionais de guardian/patient/appointment, audit (`135ae56`), encounter/clinical (`6b3df2f`) e diagnostics/care/finance/communication/knowledge/queue/AI (`bf0cc50`) com RLS contextual, joins explícitos e corrupção fail-closed | identidade/organização, jobs, cobertura universal e prova PostgreSQL concorrente continuam ausentes |
...
| 30 Supply chain | **PARTIAL** | SHA actions, SBOM, licenses, npm audit, Trivy declarado e gates locais no commit técnico `bf0cc50` | CI remoto e container scan do novo SHA ainda não foram observados como concluídos |
...
| 33 Migration safety | **PARTIAL** | migrations append-only 001–029, checksums/guards e scripts; 028 corrige o privilégio de lock em forward-only e 029 escopa DML/proveniência | dry-run, mixed-version, interruption/restart, volume representativo e prova remota ainda ausentes |
