...
| 29 Export governado | **PARTIAL/POSTGRES-BLOCKED** | `/ops/export` com purpose/TTL, PDP, idempotência, SecretProvider, AES-256-GCM, manifesto, digest e auditoria; teste local de entrega criptografada/replay | PostgreSQL/staging/secret authority e retenção/expiração operacional ainda não executados |
...
O commit `1fe6350` adiciona migration 029 para `usage_record_id`/`provenance_json`, ledger de usage idempotente e políticas DML estritas por organização/unidade/workspace; `AgentApplicationService`, harness local e persistência agora carregam e verificam referências, digests, correlation, policy revision e estado de reconciliação. O novo `ExportApplicationService` expõe `/api/v1/ops/export` com PDP `ops.export`, role admin, purpose/TTL, `Idempotency-Key`, SecretProvider e envelope AES-256-GCM; o teste local prova entrega cifrada e replay idempotente, enquanto o endpoint em memória retorna `CAPABILITY_DISABLED`.
