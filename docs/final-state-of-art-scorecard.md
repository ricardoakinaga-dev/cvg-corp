...
| 13 | Database | null | PROVEN_LOCAL | PostgreSQL 16.15 local (portátil, sem root): migrations 001–039 aplicadas; `verify:postgres` (RLS por catálogo, fence no banco, append-only, ledger de turnos do agente), `verify:postgres:concurrency` (2 processos, sem efeito duplicado) e `verify:postgres:restore` (quarentena, login/readiness bloqueados, fonte inalterada) verdes | Instância local efêmera; multi-instância staging e backup gerenciado permanecem externos |
