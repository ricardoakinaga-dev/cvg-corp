...
Nesta máquina, o gate operacional é mantido como `NOT_RUN`; o span OTLP local e o endpoint Prometheus agregado provam somente o contrato/redaction, não disponibilidade, correlação completa de métricas/logs ou SLO de produção. O Alertmanager agora exige uma configuração efetiva externa via `CVG_ALERTMANAGER_CONFIG_FILE`; [`scripts/render-alertmanager.ts`](../scripts/render-alertmanager.ts) rejeita autoridade ausente, placeholder, credenciais embutidas e sink sem TLS em produção. Sem a autoridade de alertas, o Compose falha antes do startup em vez de descartar notificações. Antes do uso real, as imagens devem ser fixadas por digest, os receivers devem operar com TLS/identidade aprovados e o canal precisa ser exercitado. O Compose e os alertas foram validados estruturalmente, mas Collector, dashboards, dispatch e exercícios de breach não foram iniciados.
