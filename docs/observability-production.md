...
Nesta máquina, o gate operacional é mantido como `NOT_RUN`; o span OTLP local prova o exporter e não disponibilidade, correlação completa de métricas/logs ou SLO de produção. Antes do uso real, as imagens devem ser fixadas por digest, os receivers devem operar com TLS/identidade aprovados e o receiver `cvg-null` do Alertmanager deve ser substituído por um canal autorizado.
