...
| Deploy e operação | Parcial | Docker/Compose/CI/runbooks, worker lifecycle e scheduler de seis lanes local; jobs/stores reais, imagens/startup/container smoke não executados |
...
Executar ambiente PostgreSQL/Docker production-like, completar observabilidade/segurança/backup, obter autoridade e provider aprovados e repetir críticas frescas. A integração local application/PDP/command boundary já está coberta na fatia de detalhe de paciente; até lá, preservar o mock, o egress fechado e os dados sintéticos.
