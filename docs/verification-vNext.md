...
| `npm test` | PASS — 100/100 | Unitário + integração, incluindo MFA, lockout, rotação, recovery manifest, PDP target-bound, Tool Gateway/ledger, providers/reconciliação, scheduler/lifecycle do worker, OTLP redaction e fault harness |
| `npm run build` | PASS | Vite web + typecheck |
| `npm run verify:static` | PASS | 39 artefatos e 115 arquivos-fonte |
...
| `npm run audit:licenses` | PASS | política SPDX aplicada a 206 dependências de terceiros, incluindo MPL-2.0 do axe-core |
