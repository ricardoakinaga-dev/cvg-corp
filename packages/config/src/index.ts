...
export const cvgConfigSchema = z.object({
  ...
  authIdentifierRateLimit: z.number().int().min(1).max(100).default(5),
  authIpRateLimit: z.number().int().min(1).max(1_000).default(30),
  ...
  embeddedModelPricingJson: z.string().trim().max(2_000).nullable().default(null),
  aiMaxCostMicros: z.number().int().nonnegative().nullable().default(null),
  ...
  if (value.embeddedModelPricingJson) {
    try {
      const parsed: unknown = JSON.parse(value.embeddedModelPricingJson);
      const pricing = parsed as Partial<Record<"inputMicrosPerToken" | "outputMicrosPerToken" | "currency" | "revision", unknown>>;
      const valid = typeof parsed === "object" && parsed !== null
        && typeof pricing.inputMicrosPerToken === "number" && pricing.inputMicrosPerToken >= 0
        && typeof pricing.outputMicrosPerToken === "number" && pricing.outputMicrosPerToken >= 0
        && typeof pricing.currency === "string" && pricing.currency.length === 3
        && typeof pricing.revision === "string" && pricing.revision.trim().length > 0;
      if (!valid) ctx.addIssue({ code: "custom", path: ["embeddedModelPricingJson"], message: "pricing must declare non-negative input/output micros per token, a 3-letter currency and a revision" });
    } catch {
      ctx.addIssue({ code: "custom", path: ["embeddedModelPricingJson"], message: "pricing must be valid JSON" });
    }
  }
});
...
export function loadCvgConfig(environment: NodeJS.ProcessEnv = process.env): CvgConfig {
  ...
  const candidate = {
    ...
    ...(environment.CVG_AUTH_IDENTIFIER_RATE_LIMIT === undefined ? {} : { authIdentifierRateLimit: Number(environment.CVG_AUTH_IDENTIFIER_RATE_LIMIT) }),
    ...(environment.CVG_AUTH_IP_RATE_LIMIT === undefined ? {} : { authIpRateLimit: Number(environment.CVG_AUTH_IP_RATE_LIMIT) }),
    ...
    ...(optionalEnvironmentValue(environment.CVG_EMBEDDED_MODEL_PRICING_JSON) === undefined ? {} : { embeddedModelPricingJson: optionalEnvironmentValue(environment.CVG_EMBEDDED_MODEL_PRICING_JSON) }),
    ...(optionalEnvironmentValue(environment.CVG_AI_MAX_COST_MICROS) === undefined ? {} : { aiMaxCostMicros: Number(environment.CVG_AI_MAX_COST_MICROS) }),
