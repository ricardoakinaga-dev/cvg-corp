...
for (const method of ["listGuardians", "listPatients", "listAppointments", "listEncounters"]) if (apiSource.includes(`persistence.${method}`) || apiSource.includes(`store.${method}`)) failures.push(`apps/api/src/app.ts: ${method} bypasses the read application service`);
