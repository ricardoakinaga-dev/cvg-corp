...
for (const method of ["listGuardians", "listPatients", "listAppointments", "listEncounters", "listDiagnosticRequests", "listSpecimens", "listDiagnosticResults"]) if (apiSource.includes(`persistence.${method}`) || apiSource.includes(`store.${method}`)) failures.push(`apps/api/src/app.ts: ${method} bypasses the read application service`);
for (const collection of ["diagnosticRequests", "specimens", "diagnosticResults"]) if (apiSource.includes(`store.${collection}.values()`)) failures.push(`apps/api/src/app.ts: ${collection} bypasses the read application service`);
